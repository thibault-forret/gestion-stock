    ...
    <div class="search-container">
        <form action="{{ route('store.invoice.search') }}" method="POST">
            ...
            <div class="buttons">
                ...
                <a class="btn red" href="{{ route('store.invoice.list') }}">{{ __('invoice.reset_search') }}</a>
            </div>
        </form>
    </div>
    ...
    <div class="filter-form">
        <form action="{{ route('store.invoice.filter') }}" method="get">
            <div class="search-element">
                ...
                <div>
                    <label for="status"><i class="fas fa-exclamation-circle"></i> {{ __('invoice.payment_status') }}</label>
    ...
    @endif
        @foreach ($invoices as $invoice)
            ...
            <div class="invoice">
                ...
                <div>
                    <a href="{{ route('store.invoice.info', ['invoice_number' => $invoice->invoice_number]) }}">{{ __('invoice.info') }}</a>
                    <a target="_blank" href="{{ route('store.order.invoice.show', ['invoice_number' => $invoice->invoice_number]) }}">{{ __('order.see_invoice') }}</a>
                    <a target="_blank" href="{{ route('store.order.invoice.download', ['invoice_number' => $invoice->invoice_number]) }}">{{ __('order.download_invoice') }}</a>
                </div>
            </div>
        @endforeach
    </div>
